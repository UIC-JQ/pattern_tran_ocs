...
from env_for_baseline import Env
import config
    ...
    for i_epoch in tqdm(range(150)):
        ...
        for t in count():
            ...
            #while env.task:
                    ...
                    # all loacl
                    _,_, reward_l,energy_l,time_l= env.step(curr_task, curr_task.n_id)
                    #print(reward_l,"111111111111")
                    lcl_reward.append(reward_l)
                    lcl_energy.append(energy_l)
                    lcl_time.append(time_l)

                    # random
                    _,_, reward_ran,energy_ran,time_ran= env.step(curr_task, random.choice([0,curr_task.n_id]))
                    ran_reward.append(reward_ran)
                    ran_energy.append(energy_ran)
                    ran_time.append(time_ran)
            ...
            if not env.task: #queue里的task耗尽，计算所有数值
                ...
                total_times_l = []
                total_energys_l = []
                total_energys_l = store_data_baseline(lcl_energy,cnt)
                total_times_l = store_data_baseline(lcl_time,cnt)
                ...
                total_times_r = []
                total_energys_r = []
                total_energys_r = store_data_baseline(ran_energy,cnt)
