...
class Node:
    def __init__(self,disk, cpu_freq, bandwidth, p_tran, p_comp,x, y):
        ...
        self.download_finish_time = 0 # 可以开始下载新的image的时间点，包括前面image下载时间和升级完成时间
        # image三种状态  0:不在本地  1:下载中  2:在本地
...
class Env:
    ...
    def reset(self):
        ...
        # 每个t时间到来几个任务
        for i in range(200): #假设一轮有200个t时间，每个时间会随机到来一些任务
            for _ in range(random.randint(0,13)):
                randn = int(np.random.normal(self.image_num // 2, 8, 1)) #在范围内生成一个随机的image
    ...
    def get_obs(self, task):
        ...
        for n in self.node:
            ...
            elif n.image_list[task.image_id] == 1:#下载了一部分
        ...
        # node_f = node_f.reshape(6,7)
        # node_f = np.transpose(node_f)
        # assert node_f.shape == (7,6)

        # task state
        #此处cpu_fre为任务需要的cpu cycles
        task_f = np.array([self.image[task.image_id].image_size, task.cpu_freq / 3, task.image_id])
 
        node_f = node_f.reshape(self.f_node)
        task_f = task_f.reshape(self.f_task)
        state = np.concatenate((node_f,task_f))
    ...
    def step(self, task, action):
        ...
        state = self.get_obs(task)
        return state, self.reward,self.per_task_energy,self.per_task_time
...
if __name__=='__main__':
    ...
    task = Task(
                    ...
                    x = 1, y = 1,
                    task_size=random.random()*config.task_size_max/1000)
    # assert len(env.get_obs(task)) == 42
    env.get_obs(task)
