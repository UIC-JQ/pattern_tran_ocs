...
class PPO():
    ...
    def update(self):
        ...
        assert state.shape == torch.Size([1000, 1, env.f_node+env.f_task]), f"the shape is{state.shape}"
        node_f = state[:, :, :env.f_node]  # shape is (1000, 1, 42)
        task_f = state[:, :, env.f_node:]   # shape is (1000, 1, 2)
        assert node_f.shape == torch.Size([1000, 1, env.f_node]), f"the shape is{state.shape}"
        node_f = node_f.view(1000,feature_dim,env.node_num).squeeze(0)
        assert node_f.shape == torch.Size([1000, feature_dim, env.node_num]), f"the shape is{state.shape}"
        ...
        for _ in range(self.ppo_epoch):
            for index in BatchSampler(
                ...
                #print(self.actor_net(state[index])[0][0]+self.actor_net(state[index])[0][1] == 1)
