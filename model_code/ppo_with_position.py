...
class Transformer_Model(nn.Module):
    ...
    def forward(self, node_f, task_f, b_s):
        ...
        #positional_encoding
        norm_x = norm_x.reshape(-1,b_s,feature_dim)
        norm_x = self.pos_enc(norm_x)
        exit(0)
        #transformer
    ...
    for i_epoch in tqdm(range(150)):
        ...
        for t in count():
            ...
            if not env.task: #queue里的task耗尽，计算所有数值
                ...
                print("final result energy cost: ",min(save_energy_ep))
                print("final result time cost: ",min(save_time_ep))
                break
